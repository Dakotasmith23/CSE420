  	...
  	else
  	{
			...
			if(strcmp(entity->d_name, ".")!=0 && strcmp(entity->d_name, "..")!=0)
			{
			  ...
			  if(entity->d_type != DT_DIR) // parseFile and insert
			  {
				  ...
				  {
				  	tmp = create_node(entityPath, entityLevel);
					  tmp->entityPath = strdup(entityPath);//memory leak here. idk why??
					  tmp->keyword = keyword;
					  //tmp->entityPath = entityPath; this sets it to the same entityPath each loop iteration for some reason?
					  insert_sorted(tmp,list);								  
				  }
...
int main(int argc, char *argv[]) {
  ...
  // Pretty sure this is the correct way to initialize the
  //char *initPath to first cmd
  ...
  struct List *list = create_list();
	putEntitiesInList(argv[1],initLevel, list, argv[2],argv[4]); // argv[1] will be a pathname
  ...
  if(strcmp(argv[4],"1")==0)
  {
  	...
  	while(ptr!=NULL)
  	{
  		if(ptr->isDir==0)//if not directory then parse
  		{
  			pthread_t tid;
  			pthread_create(&tid,&attr,parseFileParallel,(void *)ptr);
  			ptr->tid = tid;
   		}
  	...
  	while(ptr!=NULL)
  	{
  		if(ptr->isDir==0)//if not directory then join
  		{
  			// before printf line below executes there is this error
  			/*
  			Thread 1 "pardirlist" received signal SIGSEGV, Segmentation fault.
0x00007ffff7f9baab in __pthread_clockjoin_ex (threadid=64, thread_return=0x0, clockid=0, abstime=0x0, block=true) at pthread_join_common.c:89
